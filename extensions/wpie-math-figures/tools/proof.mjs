...
// The formula engine is fetched from assets/ next to extension.js; on the file:// stage that is the stage itself.
...
const { page } = qa;
await page.waitForFunction( () => window.__wpiemfProof && window.__wpiemfState && window.__wpiemfState().frames > 0, null, { timeout: 30000 } );
const only = process.env.PROOF_ONLY ? process.env.PROOF_ONLY.split( ',' ) : null;
const starters = await page.evaluate( () => window.__wpiemfProof.starters );
const tiles = [];
for ( const s of starters ) {
	...
	try {
		res = await page.evaluate( async ( id ) => {
			const r = await window.__wpiemfProof.render( id );
	...
	console.log( s.id.padEnd( 20 ), ( s.group || '' ).padEnd( 10 ), res.width + 'x' + res.height, ( Date.now() - t0 ) + ' ms', 'texts ' + res.texts, res.classes ? 'CLASSES ' + res.classes : '', res.warnings.length ? 'WARN ' + res.warnings.join( ' | ' ) : '' );
